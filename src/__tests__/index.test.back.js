...
describe.skip('next-api', () => {
  ...
  function initResponses() {
    ...
    sandbox.server.respondWith('/next/2/ping', 'pong');
  }
  ...
  describe('when Content-type is not JSON', () => {
    beforeEach(settle(get, '/next/2/ping'));

    it('should return expected status', () => expect(response.status).to.equal(200));
    it('should return expected response data', () => expect(response.data).to.equal('pong'));
  });
});
